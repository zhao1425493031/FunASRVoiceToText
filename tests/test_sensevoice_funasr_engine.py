...
def test_language_kw_maps_codes(meeting_config, lang, expected) -> None:
    assert _language_kw(lang, meeting_config.language) == expected


def test_language_kw_override_beats_default(meeting_config) -> None:
    assert _language_kw("zh", "ja") == "zh"


@patch("funasr.AutoModel")
def test_transcribe_calls_generate(mock_auto, meeting_config) -> None:
    model = MagicMock()
    model.generate.return_value = [{"text": "测试"}]
    mock_auto.return_value = model

    engine = SenseVoiceFunASREngine(meeting_config)
    engine.load()
